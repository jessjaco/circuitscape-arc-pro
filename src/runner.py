...
def run_ascape(get_script: Callable, config_file: Path, messages) -> None:
    ...
    julia_script = script_text(wrking_dir, cfg_file)
...
def run_omniscape(config_file: Path, messages) -> None:
    cs_script = lambda working_dir, config_file: f'"using Pkg; Pkg.activate(\\"{working_dir}\\"); Pkg.instantiate(); using Omniscape; run_omniscape(\\"{config_file}\\")"'
    return run_ascape(julia_script, config_file, messages)

def run_circuitscape(config_file: Path, messages) -> None:
    cs_script = lambda working_dir, config_file: f'"using Pkg; Pkg.activate(\\"{working_dir}\\"); Pkg.instantiate(); using Circuitscape; compute(\\"{config_file}\\")"'
    return run_ascape(julia_script, config_file, messages)